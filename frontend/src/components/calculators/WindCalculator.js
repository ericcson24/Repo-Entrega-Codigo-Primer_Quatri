...
import { Calculator, MapPin, DollarSign, Wind } from 'lucide-react';

const WindCalculator = ({ onSimulate }) => {
    ...
    const handleEquipmentSelect = (item) => {
        if (item) {
            setFormData(prev => ({ ...prev, turbine_id: item.id }));
        } else {
            setFormData(prev => ({ ...prev, turbine_id: '' }));
        }
    };
    ...
    return (
        <div className="max-w-2xl mx-auto">
            <div className="mb-8">
                <h2 className="text-2xl font-bold text-white mb-2">Wind Power Simulation</h2>
                <p className="text-gray-400">Professional assessment for small to medium scale wind projects.</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {/* Equipment Section */}
                <div className="card-panel">
                     <EquipmentSelector type="wind" onSelect={handleEquipmentSelect} />

                     <div className="mb-4">
                        <label className="block text-xs text-gray-500 mb-1">Number of Turbines</label>
                        <input 
                            type="number" 
                            min="1"
                            className="input-field"
                            value={formData.num_turbines}
                            onChange={e => setFormData({...formData, num_turbines: Number(e.target.value)})} 
                        />
                     </div>
                </div>
